...
if __name__ == '__main__':
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C57ROM1.txt', 'rb') as file:
        lines = file.readlines()
    with open(Path(__file__).parent / 'CDATA' / 'C57ROM2.txt', 'rb') as file:
        lines.extend(file.readlines())
    with open(Path(__file__).parent / 'CDATA' / 'C57ROM3.txt', 'rb') as file:
        lines.extend(file.readlines())
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C57ROM0.txt', 'rb') as file:
        lines = file.readlines()
    ...
    for raw_entry in raw_entries:
        try:
            ...
            print(f"Unknown genre code: {raw_entry=}")
            raise e
        name = cleanup(raw_entry[7])
        pen_name = cleanup(raw_entry[8])
        product_name = cleanup(raw_entry[9])
