...
DECODE_INDEX: dict[bytes, bytes] = { # Map bytes that are not decoded properly to their correct characters
    b'\x88h': '・'.encode('cp932'),
    b'\x98|': '・'.encode('cp932'),
    b'\x86\xa5': '・'.encode('cp932'),
}
GENRES: dict[int, str] = {
    10: "×××× (Probable misinput, guess: 創作(少年))", # see 'Ｒｅ－Ｌａｘ'
    11: "×××× (Probable misinput, guess: 創作(少女))", # see 'ＤＥＭＩＴＡＳＳ'
    16: "×××× (Probable misinput, guess: 評論・情報)", # see '怪汁研究所'
    20: "×××× (Probable misinput, guess: 創作・アニメ・ケーム(男性向))", # see '河豚提灯
    35: "×××× (Probable misinput, guess: ケーム(電源不要&オンライン))", # see '桜蘭帝國'
    71: "×××× (Probable misinput, guess: 音楽(洋楽・邦楽))", # see 'ＥＸＥ．'
    80: "×××× (Probable misinput, guess: FC(小説))", # see 'ＶＩＶＡＣＨ■'
    88: "×××× (Probable misinput, guess: 冨樫義博)", # see 'Ｈｅａｖｅｎｓ'
    999: "××××", # see 'Ｃ．Ｐ．Ｍ．'
    100: "創作(少年)",
    101: "創作(少女)",
    102: "創作(JUNE)",
...
if __name__ == '__main__':
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C60ROM1.txt', 'rb') as file:
        lines = file.readlines()
    with open(Path(__file__).parent / 'CDATA' / 'C60ROM2.txt', 'rb') as file:
        lines.extend(file.readlines())
    with open(Path(__file__).parent / 'CDATA' / 'C60ROM3.txt', 'rb') as file:
        lines.extend(file.readlines())
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C60ROM0.txt', 'rb') as file:
        lines = file.readlines()
