...
DECODE_INDEX: dict[bytes, bytes] = { # Map bytes that are not decoded properly to their correct characters
    ...
    b'\x86\xa1': '・'.encode('cp932'),
}
GENRES: dict[int, str] = {
    201: "---- (Probably forgotten, no genre guess)", # see 'Ｆａｉｒｙ  Ｐｒｏｊｅｃｔ'
    999: "----", # see 'STUDIO LAGRANGE･POINT):'
    100: "創作(少年)",
    101: "創作(少女)",
    102: "創作(JUNE)",
    105: "学漫",
...
if __name__ == '__main__':
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C61ROM1.txt', 'rb') as file:
        lines = file.readlines()
    with open(Path(__file__).parent / 'CDATA' / 'C61ROM2.txt', 'rb') as file:
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C61ROM0.txt', 'rb') as file:
        lines = file.readlines()
