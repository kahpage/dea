...
GENRES: dict[int, str] = {
    ...
    204: "同人ソフト",
    207: "Leaf & Key",
    208: "ギャルゲー(PC)(初出がバソコン)",
    209: "ギャルグー(CS)(初出が家庭用グーム機)",
    302: "ゲーム(格闘)",
    305: "ケーム(電源不要&オンライン)",
    308: "ケーム(その他)",
    317: "ケーム(RPG)",
...
if __name__ == '__main__':
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C62ROM1.txt', 'rb') as file:
        lines = file.readlines()
    with open(Path(__file__).parent / 'CDATA' / 'C62ROM2.txt', 'rb') as file:
        lines.extend(file.readlines())
    with open(Path(__file__).parent / 'CDATA' / 'C62ROM3.txt', 'rb') as file:
        lines.extend(file.readlines())
    ...
    # ===== Load csv like data ======
    with open(Path(__file__).parent / 'CDATA' / 'C62ROM0.txt', 'rb') as file:
        lines = file.readlines()
